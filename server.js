...
//  Node.js + Express
//  New: AI weather chat, spotter network, severe analysis, forecast sounding
// ================================================================
...
// ================================================================
//  SEVERE ANALYSIS  — computes risk tags from forecast data
// ================================================================
app.get('/api/severe-analysis', async (req, res) => {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng))
    return res.status(400).json({ error: 'valid lat and lng required' });
  try {
    // Fetch extended forecast with severe parameters
    const url = 'https://api.open-meteo.com/v1/forecast'
      + `?latitude=${lat}&longitude=${lng}`
    ...
    function analyzeSevereRisk(h) {
      ...
      const windMph = (h.windMs || 0) * 2.237;
      // Convective
    ...
    const peakHours = analyzed.filter(h => h.tags.length > 0)
      .sort((a, b) => b.cape - a.cape || b.tags.length - a.tags.length)
      .slice(0, 6);
...
// ================================================================
//  AI WEATHER ASSISTANT  — powered by Anthropic Claude
// ================================================================
app.post('/api/ai-chat', async (req, res) => {
  if (!ANTHROPIC_KEY) {
    return res.status(503).json({ error: 'AI assistant not configured. Add ANTHROPIC_API_KEY to .env' });
  }
  ...
  const systemPrompt = `You are Storm Surge AI, an expert meteorological assistant embedded in the Storm Surge Weather app. You have access to real-time weather data for the user's current location and can analyze radar, forecast data, severe weather parameters, and spotter reports.
...
Weather code: ${context.weather.current?.weather_code}
...
Cloud cover: ${context.weather.current?.cloud_cover}%
...
${context?.severe ? `Severe analysis:
...
${context?.alerts && context.alerts.length > 0 ? `Active NWS alerts (${context.alerts.length}):
${context.alerts.slice(0,3).map(a => `- ${a.properties?.event}: ${a.properties?.headline || ''}`).join('\n')}` : 'No active NWS alerts.'}

${context?.spotterReports && context.spotterReports.length > 0 ? `Recent spotter reports (${context.spotterReports.length}):
${context.spotterReports.slice(0,5).map(r => `- ${r.type}: ${r.description || r.comments || ''} at ${r.city || ''}, ${r.state || ''}`).join('\n')}` : ''}

Instructions:
- Answer weather questions concisely and accurately
- Use the provided real-time data to give specific, location-aware answers
- For severe weather questions, analyze the CAPE, shear, and instability data
- When asked about radar, describe what the current weather parameters suggest
- Give practical advice (e.g. "comfortable for a walk", "bring an umbrella", "stay indoors")
- Be direct and confident but acknowledge forecast uncertainty when relevant
- Use weather emoji where appropriate to make responses scannable
- Keep responses under 200 words unless a detailed analysis is requested`;
  ...
  try {
    ...
    if (!r.ok) {
      ...
      return res.status(502).json({ error: 'AI request failed', detail: err.error?.message || r.status });
    }
    const data = await r.json();
...
app.get('/api/spotter-reports', async (req, res) => {
  ...
  // 1) mPing — NSSL crowdsourced weather reports (public API, no key needed)
  try {
    const now  = Math.floor(Date.now() / 1000);
    const from = now - 3 * 3600; // last 3 hours
    const mUrl = `https://mping.nssl.noaa.gov/mping/api/v2/reports/?format=json&time__gte=${from}&limit=200`;
    const mr   = await fetch(mUrl, {
      headers: { 'User-Agent': 'StormSurgeWeather/13.8', 'Accept': 'application/json' }
    });
    if (mr.ok) {
      const md = await mr.json();
      const categoryMap = {
        1: { type: 'Rain', icon: '🌧' }, 2: { type: 'Freezing Rain', icon: '🌨' },
        3: { type: 'Snow', icon: '❄️' }, 4: { type: 'Ice Pellets/Sleet', icon: '🧊' },
        5: { type: 'Hail', icon: '🧭' }, 6: { type: 'Tornado', icon: '🌪' },
        7: { type: 'Thunderstorm', icon: '⛈' }, 8: { type: 'Fog', icon: '🌫' },
        9: { type: 'High Wind', icon: '💨' }, 10: { type: 'Blizzard', icon: '🌨' },
        11: { type: 'Lightning', icon: '⚡' }, 12: { type: 'Funnel Cloud', icon: '🌀' },
        13: { type: 'Flash Flood', icon: '🌊' }, 14: { type: 'Waterspout', icon: '🌀' },
        15: { type: 'Dense Fog', icon: '🌫' }, 16: { type: 'Drizzle', icon: '🌦' },
        17: { type: 'Freezing Fog', icon: '🌫' }, 18: { type: 'Dust Storm', icon: '💨' }
      };
      (md.results || []).forEach(rpt => {
        ...
        reports.push({
          ...
          type:   cat.type,
          icon:   cat.icon,
          lat:    rpt.geom?.coordinates?.[1] || 0,
          lng:    rpt.geom?.coordinates?.[0] || 0,
          description: rpt.description || '',
          comments:    rpt.comments    || '',
          city:   rpt.city   || '',
          state:  rpt.state  || '',
          ts:     rpt.ob_time || new Date().toISOString(),
          verified: false
        });
      });
    }
  } catch (e) {
    console.warn('mPing fetch failed:', e.message);
  }

  // 2) NWS Local Storm Reports (LSR) — SPC today's LSR CSV
  try {
    const lsrTypes = [
    ...
    for (const src of lsrTypes) {
      ...
      lines.slice(0, 60).forEach((line, i) => {
        const p = line.split(',');
        const rlat = parseFloat(p[5]) || 0;
        const rlng = parseFloat(p[6]) || 0;
        if (!rlat || !rlng) return;
        reports.push({
          id:     `spc-${src.type.toLowerCase()}-${i}`,
          source: 'SPC/NWS',
          type:   src.type,
          icon:   src.icon,
          lat:    rlat,
          lng:    rlng,
          description: `${src.type} report — ${p[3] || '?'} magnitude`,
          comments:    p[7] || '',
          city:   (p[8] || '').trim(),
          state:  (p[4] || '').trim(),
    ...
    console.warn('SPC LSR fetch failed:', e.message);
  }
  ...
  const nearby = lat && lng
    ? reports.filter(r => r.lat && r.lng && haversine(lat, lng, r.lat, r.lng) <= dist)
    : reports;
  nearby.forEach(r => {
    if (lat && lng && r.lat && r.lng) r.distKm = Math.round(haversine(lat, lng, r.lat, r.lng));
  });
